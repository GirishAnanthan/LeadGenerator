...
async function scrapeGoogleSearchPaginated(browser, query, existingDomains, onLeadFound, onStatusUpdate, isCancelledFn, searchDepth = SEARCH_DEPTH.MEDIUM) {
  ...
  try {
    while (!isCancelledFn()) {
      ...
      await safeGoto(searchPage, url, { waitUntil: WAIT_STRATEGY.NETWORK, timeout: TIMEOUT.MEDIUM });
      ...
      await processConcurrently(uniqueLinks, CONCURRENCY.HIGH, async (link) => {
        ...
        try {
          ...
          await safeGoto(webPage, link.url, { waitUntil: WAIT_STRATEGY.NETWORK, timeout: TIMEOUT.MEDIUM });
