...
async function scrapeLeads({ countryCode, country, state, city, industry, searchDepth = 'deep' }, onLeadFound, onStatusUpdate, isCancelledFn = () => false) {
  let browser;
  try {
    ...
    onStatusUpdate(`Search mode: ${searchDepth.toUpperCase()}. ${searchDepth === 'fast' ? 'Only Maps + VIBE' : searchDepth === 'medium' ? 'Maps + Google Search' : 'Full deep search with variations + directories'}...`);

    // === COMMON PHASES (all depths) ===

    // --- Phase 1: Vibe Prospecting API ---
    try {
    ...
    // --- Phase 2: Google Maps (1 query, 50 scrolls) ---
    ...
    if (searchDepth === 'deep') {
      ...
      // --- Phase 5: Multi-query Search (variations + B2B sites) ---
      if (!isCancelledFn()) {
        onStatusUpdate('Searching with 15+ query variations and B2B directories...');
      ...
      // --- Phase 6: Directory Discovery (slowest) ---
      if (!isCancelledFn()) {
